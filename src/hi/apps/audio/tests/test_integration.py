...
class TestAudioIntegration(BaseTestCase):
    ...
    def test_weather_alert_gets_weather_specific_audio(self):
        ...
        weather_alarm = Alarm(
            ...
            sensor_response_list=[SensorResponse(integration_key=IntegrationKey("test", "audio_test"), value="active", timestamp=datetimeproxy.now(), sensor=None, detail_attrs={'location': 'Austin, TX'}, event_video_snapshot_url=None, has_event_video_clip=False)],
    ...
    def test_event_alert_gets_event_specific_audio(self):
        ...
        system_alarm = Alarm(
            ...
            sensor_response_list=[SensorResponse(integration_key=IntegrationKey("test", "audio_test"), value="active", timestamp=datetimeproxy.now(), sensor=None, detail_attrs={'device': 'Sensor-01'}, event_video_snapshot_url=None, has_event_video_clip=False)],
            security_level=SecurityLevel.OFF,
    ...
    def test_same_level_different_source_gets_different_audio(self):
        ...
        system_alarm = Alarm(
            ...
            sensor_response_list=[SensorResponse(integration_key=IntegrationKey("test", "audio_test"), value="active", timestamp=datetimeproxy.now(), sensor=None, detail_attrs={'sensor': 'Temp-01'}, event_video_snapshot_url=None, has_event_video_clip=False)],
            security_level=SecurityLevel.OFF,
            alarm_lifetime_secs=3600,
            timestamp=datetimeproxy.now(),
        )
        ...
        for alarm_source, alarm_level, expected_signal in test_cases:
            with self.subTest(source=alarm_source, level=alarm_level):
                alarm = Alarm(
                    ...
                    title='Test Alarm',
                    sensor_response_list=[SensorResponse(integration_key=IntegrationKey("test", "audio_test"), value="active", timestamp=datetimeproxy.now(), sensor=None, detail_attrs={'test': 'data'}, event_video_snapshot_url=None, has_event_video_clip=False)],
                    security_level=SecurityLevel.OFF,
                    alarm_lifetime_secs=3600,
                    timestamp=datetimeproxy.now(),
                )

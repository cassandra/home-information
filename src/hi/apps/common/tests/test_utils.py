...
class TestStripParentNamePrefix(TestCase):
    ...
    def test_strips_with_space_separator(self):
        self.assertEqual(
            utils.strip_parent_name_prefix( 'Zoo Heater Fan Mode', 'Zoo Heater' ),
            'Fan Mode',
        )
    ...
    def test_returns_full_when_no_prefix_match(self):
        self.assertEqual(
            utils.strip_parent_name_prefix( 'Outdoor Temperature', 'Zoo Heater' ),
            'Outdoor Temperature',
        )

    def test_returns_full_when_stripping_leaves_nothing(self):
        self.assertEqual(
            utils.strip_parent_name_prefix( 'Zoo Heater', 'Zoo Heater' ),
            'Zoo Heater',
        )

    def test_returns_full_when_parent_name_empty(self):
        self.assertEqual(
            utils.strip_parent_name_prefix( 'Zoo Heater Fan Mode', '' ),
            'Zoo Heater Fan Mode',
        )

    def test_returns_empty_string_when_full_name_empty(self):
        self.assertEqual(
            utils.strip_parent_name_prefix( '', 'Zoo Heater' ),
            '',
        )

    def test_handles_none_full_name(self):
        self.assertEqual(
            utils.strip_parent_name_prefix( None, 'Zoo Heater' ),
            '',
        )

...
class TestViewUrlUtils(BaseTestCase):
    ...
    def test_motion_sensor_with_video_stream_generates_url(self):
        ...
        with patch('django.urls.reverse') as mock_reverse:
            mock_reverse.return_value = f'/console/sensor/video_stream/{self.motion_sensor.id}/'
            
            view_url = ViewUrlUtils.get_view_url_for_alarm(alarm)
            
            self.assertEqual(view_url, f'/console/sensor/video_stream/{self.motion_sensor.id}/')
            mock_reverse.assert_called_once_with(
                'console_sensor_video_stream',
                kwargs={'sensor_id': str(self.motion_sensor.id)}
            )
    ...
    def test_video_sensor_generates_url(self):
        ...
        with patch('django.urls.reverse') as mock_reverse:
            mock_reverse.return_value = f'/console/sensor/video_stream/{self.video_sensor.id}/'
            
            view_url = ViewUrlUtils.get_view_url_for_alarm(alarm)
            
            self.assertEqual(view_url, f'/console/sensor/video_stream/{self.video_sensor.id}/')

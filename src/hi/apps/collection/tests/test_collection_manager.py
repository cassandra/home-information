...
class TestCollectionManagerIntegration(BaseTestCase):
    ...
    def test_collection_data_generation_includes_entity_status_and_svg_icons(self):
        ...
        self.assertEqual(len(collection_data.entity_panel_data_list), 2)

        # Verify entities are ordered correctly
        entity_names = [panel_data.entity.name for panel_data in collection_data.entity_panel_data_list]
        self.assertEqual(entity_names, ['Security Camera', 'Porch Light'])

        # Verify each card has the expected entity-derived fields
        for entity_panel_data in collection_data.entity_panel_data_list:
            self.assertIsNotNone(entity_panel_data.entity)
            self.assertIsNotNone(entity_panel_data.display_only_svg_icon_item)
            # SVG icon should be configured for display
            self.assertIsNotNone(entity_panel_data.display_only_svg_icon_item.template_name)
...
class TestCollectionManager(BaseTestCase):
    ...
    def test_collection_data_enhanced_entity_display_categories(self):
        ...
        self.assertIn('entity_panel_data_list', context)
        self.assertEqual(context['collection'], collection)
        self.assertEqual(len(context['entity_panel_data_list']), 4)

        # Verify entity_panel_data_list contains the expected entities
        entity_names = [panel_data.entity.name for panel_data in context['entity_panel_data_list']]

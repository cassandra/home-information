...
class AttributeEditFormHandler:
    ...
    def create_edit_form_data( self,
        ...
        """
        owner_form = attr_context.create_owner_form( form_data )
        
        # Get file attributes for display (not a formset, just for template rendering)
        file_attributes: QuerySet[AttributeModel] = attr_context.attributes_queryset().filter(
            value_type_str = str( AttributeValueType.FILE )
        ).order_by('id')
        
        # Regular attributes formset (should exclude FILE attributes)
        regular_attributes_formset = attr_context.create_regular_attributes_formset(
            form_data = form_data,
        )
    ...
    def process_file_deletions( self,
        ...
        AttributeModelClass = attr_context.attribute_model_subclass
    ...
    def process_file_title_updates( self,
        ...
        for field_name, new_title in request.POST.items():
            ...
            if int(owner_id_str) != attr_context.owner.id:
            ...
            except (ValueError, AttributeModelClass.DoesNotExist) as e:

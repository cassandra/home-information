...
@register.filter
def file_title_field_name(attr_context, attribute_id):
    """
    Generate the form field name for file title editing.
    
    Usage in template: {{ attr_context|file_title_field_name:attribute.id }}
    
    Args:
        attr_context: AttributeEditContext instance
        attribute_id: The attribute's ID
        
    Returns:
        str: Form field name like 'file_title_1_23'
    """
    return attr_context.file_title_field_name(attribute_id)


@register.filter
def history_target_id(attr_context, attribute_id):
    """
    Generate the DOM ID for attribute history container.
    
    Usage in template: {{ attr_context|history_target_id:attribute.id }}
    ...
    """
    return attr_context.history_target_id(attribute_id)


@register.filter
def history_toggle_id(attr_context, attribute_id):
    """
    Generate the DOM ID for history toggle/collapse target.
    
    Usage in template: {{ attr_context|history_toggle_id:attribute.id }}
    ...
    """
    return attr_context.history_toggle_id(attribute_id)


@register.simple_tag
def attr_history_url(attr_context, attribute_id):
    """
    Generate URL for attribute history view with correct parameter names.
    
    Usage in template: {% attr_history_url attr_context attribute.id %}
    ...
    url_name = attr_context.history_url_name
    params = {
        attr_context.owner_id_param_name: attr_context.owner_id,
...
@register.simple_tag
def attr_restore_url(attr_context, attribute_id, history_id):
    """
    Generate URL for attribute restore view with correct parameter names.
    
    Usage in template: {% attr_restore_url attr_context attribute.id history_record.pk %}
    ...
    """
    from django.urls import reverse
    url_name = attr_context.restore_url_name
    params = {
        attr_context.owner_id_param_name: attr_context.owner_id,
        'attribute_id': attribute_id,

...
class IntegrationManagerTestCase(TestCase):
    ...
    def test_create_integration_attribute(self):
        """Test creation of individual integration attribute."""
        manager = IntegrationManager()
        ...
        # Call method
        manager._create_integration_attribute(
            integration=integration,
            attribute_type=MockIntegrationAttributeType.TEST_ATTR
        )
        
        # Verify attribute was created correctly
        self.assertEqual(integration.attributes.count(), 1)
        
        attr = integration.attributes.first()
        self.assertEqual(attr.integration, integration)
        self.assertEqual(attr.name, 'Test Attribute')
        self.assertEqual(attr.value, 'default')
        self.assertEqual(attr.value_type_str, str(AttributeValueType.TEXT))
        self.assertEqual(attr.attribute_type_str, str(AttributeType.PREDEFINED))
        self.assertTrue(attr.is_editable)
        self.assertTrue(attr.is_required)
        
        # Verify integration key format
        expected_key = f'test_integration.{str(MockIntegrationAttributeType.TEST_ATTR).lower()}'
        self.assertEqual(attr.integration_key_str, expected_key)
        
        # Verify value_range_str is JSON
        import json
        self.assertEqual(json.loads(attr.value_range_str), {})

    def test_enable_integration_database_transaction(self):
        """Test enable integration with database transaction and monitor launch."""
        manager = IntegrationManager()
        ...
        with patch.object(manager, '_launch_integration_monitor_task') as mock_launch:
            # Call enable
            manager.enable_integration(data, mock_formset)
            
            # Verify database changes
            integration.refresh_from_db()
            self.assertTrue(integration.is_enabled)
            
            # Verify formset was saved
            mock_formset.save.assert_called_once()
            
            # Verify monitor was launched

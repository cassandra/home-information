...
class IntegrationAttributeReferencer( CapabilityGateway ):
    ...
    def search_references(
        ...
        candidates. Operators see the returned list rendered as
        cards (thumbnail/mime-icon + title + snippet + clickable
        source URL); multi-selecting any subset attaches them as
        TEXT attributes on the host Entity or Location.

        Implementations should:
          - Return an empty list when the query yields no matches.
